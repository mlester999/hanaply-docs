...
function CareerProfile() {
  return <section className="constellation-section section-pad"><div className="content-shell"><SectionIntro index="03" eyebrow="Career profile constellation" title={<>A career is more than<br /><em>a keyword list.</em></>} copy="Hanaply is designed to understand a connected body of verified experience, skills, projects, tools, preferences, and goals." light /><div className="constellation-layout"><CareerConstellationScene /><div className="constellation-copy"><span className="eyebrow">How evidence behaves</span><h3>A job activates only the facts that matter.</h3><p>A sample Automation Engineer role lights up relevant tools, delivery experience, and projects. Missing requirements stay separate. Hard blockers never hide behind an average score.</p><dl><div><dt><i className="mint" />Verified match</dt><dd>Evidence directly supports a requirement.</dd></div><div><dt><i className="blue" />Transferable</dt><dd>Related experience can carry into the role.</dd></div><div><dt><i className="coral" />Hard blocker</dt><dd>A constraint makes the role unrealistic now.</dd></div></dl></div></div></div></section>;
}
...
function MobileFuture() {
  return <section id="mobile" className="mobile-section section-pad"><div className="content-shell"><SectionIntro index="13" eyebrow="Mobile future" title={<>Built for the web first.<br /><em>Designed for mobile from day one.</em></>} copy="The planned React Native apps will connect to the same authentication, entitlements, career intelligence, documents, and event system." light /><div className="mobile-orbit"><div className="platform-core"><span className="brand-mark"><i /><i /><i /></span><strong>Shared Hanaply platform</strong><small>Auth · Entitlements · Career Intelligence</small></div><div className="orbit-ring ring-one" /><div className="orbit-ring ring-two" /><article className="surface web"><Globe2 size={21} /><span>Web SaaS</span><small>First platform</small></article><article className="surface ios"><Smartphone size={21} /><span>React Native iOS</span><small>Future phase</small></article><article className="surface android"><Smartphone size={21} /><span>React Native Android</span><small>Future phase</small></article><div className="mobile-capabilities"><span>Push notifications</span><span>Deep links</span><span>Minimum versions</span><span>Platform flags</span></div></div><p className="demo-disclaimer inverse"><Smartphone size={15} /> Mobile applications are future product direction, not currently available.</p></div></section>;
}
