...
export function ClaimVerificationDemo() {
  ...
  return <div className="truth-demo">
    ...
    <div className={`claim-card ${blocked ? "blocked" : "safe"}`}><span className="small-label">AI-assisted sentence</span><p>{blocked ? "Architected enterprise infrastructure for a Fortune 500 company." : "Developed an automated workflow integrating OpenAI, Supabase, and Gmail to reduce repetitive operational work."}</p><div>{blocked ? <><span className="block-icon">×</span><strong>Unsupported claim removed</strong><small>This statement is not backed by confirmed profile details.</small></> : <><Check size={17} /><strong>Supported by the profile</strong><small>The sentence uses only confirmed career details.</small></>}</div><button type="button" onClick={() => setBlocked((value) => !value)}>{blocked ? "Show supported version" : "Show an unsupported version"}</button></div>
  </div>;
}
